...
-- TODO Add completeness checker and thus an error stack for F
module Generate (
  compile,
  recompile,
  genSystem,
...
lowerHIR :: CompilationUnit HIR -> F (CompilationUnit IR)
lowerHIR c = do
...
lowerIR :: CompilationUnit IR -> F (CompilationUnit LIR)
lowerIR c = do
...
emit :: CompilationUnit LIR -> F (CompilationUnit Prim)
emit c = do
  (length, prim) <- execRWST (mapM_ emit' (c ^. unitIR)) () 0
  pure $ c { _unitIR = prim }
  where emit' :: LIR -> RWST () [Prim] Int16 F ()
...
resolve :: CompilationUnit Prim -> F (CompilationUnit Int16)
resolve c = do
...
-- | Generate the recompilation command stream
genRecomp :: CompilationUnit Int16 -> F ()
genRecomp c = do
  sys <- asks fromJust
  case M.lookup (c^.unitName) (sys^.imageTable) of
    Nothing -> throwError $ NewUnit $ c^.unitName
    Just loc -> tellI loc >> tellI (fromIntegral $ length $ c^.unitIR)
  mapM_ tellI $ c^.unitIR

-- * Compiler evaluation

-- | Default compiler sequence
compile :: AST -> Either Err (VMState, String)
compile ast = compileAST Nothing (VMState 0 M.empty M.empty S.empty) ast genSystemMap cont
  where cont = do
          m <- use localSymbols
...
recompile :: AST -> System -> Either Err (VMState, String)
recompile ast sys = compileAST (Just sys) (VMState 0 M.empty M.empty S.empty) ast genRecomp cont
  where cont = do
          st <- use symbolTable
          ls <- use localSymbols
          let sym = sys^.symbols
              fsym = S.filter (\(cu, _) -> cu `elem` map _unitName (ast^.comps)) sym
              symbs = M.union ls $ sys^.lastSymbols
          unless (fsym `S.isSubsetOf` st) $ throwError $ MissingSymbols (fsym `S.difference` st)
          tellI (-1)
          mapM_ tellI $ map (symbs M.!) $ S.toAscList sym
          tellI (-2)
...
compileAST :: Maybe System -- ^ The system for which to compile
           -> VMState -- ^ The precompiled state if any
           -> AST -- ^ The ast currently compiled for
           -> (CompilationUnit Int16 -> F ()) -- ^ The generator
           -> F a -- ^ Continuation
           -> Either Err (VMState, String)
compileAST system state (AST inls cus) gen cont = runExcept $ execRWST compile' system state
  where compile' = do
          case system of
            Just sys  -> nativeCalls .= (sys ^. lastNativeCalls)
            Nothing -> genInlines inls
