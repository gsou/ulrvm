...
recompile :: System -> AST -> Either Err (VMState, String)
recompile _ (AST (a:_) _) = Left (CantDefSymbol $ fst a)
recompile system ast@(AST [] [c]) = compileAST (Just system) (VMState 0 M.empty M.empty S.empty) ast gen $ pure ()
  where gen c = do
          sys <- asks fromJust
          case M.lookup (c^.unitName) (sys^.imageTable) of
            Nothing -> throwError $ NewUnit $ c^.unitName
            Just loc -> tellS loc >> tellS (fromIntegral $ length $ c^.unitIR)
          mapM_ tellS $ c^.unitIR
